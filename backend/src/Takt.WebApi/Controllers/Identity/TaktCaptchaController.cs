...
public class TaktCaptchaController : ControllerBase
{
        ...
        try
        {
            ...
            if (request?.UserInput != null)
            {
                TaktLogger.Debug("[Captcha Verify] 用户输入详情: {UserInputJson}", 
                    System.Text.Json.JsonSerializer.Serialize(request.UserInput));
            }
            ...
            if (!result.Success && result.Message.Contains("验证码不存在"))
            {
