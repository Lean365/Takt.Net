...
using Takt.Application.Dtos.Routine.WordFilter;
...
namespace Takt.WebApi.Controllers.Routine.WordFilter;
...
[TaktPermission("routine:wordfilter", "敏感词过滤管理")]
public class TaktWordFiltersController : TaktControllerBase
{
    ...
    [HttpPost("check")]
    [TaktPermission("routine:wordfilter:check", "检查敏感词")]
    ...
    public ActionResult<TaktApiResult<CheckTextResultDto>> CheckText([FromBody] CheckTextDto dto)
    {
        try
        {
            ...
            {
                return Ok(TaktApiResult<CheckTextResultDto>.Fail("文本不能为空"));
            }
        ...
        {
            return Ok(TaktApiResult<CheckTextResultDto>.Fail($"检查失败: {ex.Message}"));
        }
    }
    ...
    [HttpPost("find")]
    [TaktPermission("routine:wordfilter:find", "查找敏感词")]
    ...
    public ActionResult<TaktApiResult<FindWordsResultDto>> FindWords([FromBody] FindWordsDto dto)
    {
        try
        {
            if (string.IsNullOrWhiteSpace(dto.Text))
            {
                return Ok(TaktApiResult<FindWordsResultDto>.Fail("文本不能为空"));
            }
        ...
        {
            return Ok(TaktApiResult<FindWordsResultDto>.Fail($"查找失败: {ex.Message}"));
        }
    }
    ...
    [HttpPost("replace")]
    [TaktPermission("routine:wordfilter:replace", "替换敏感词")]
    [AllowAnonymous] // 允许匿名访问
    public ActionResult<TaktApiResult<ReplaceWordsResultDto>> ReplaceWords([FromBody] ReplaceWordsDto dto)
    {
        try
        {
            if (string.IsNullOrWhiteSpace(dto.Text))
            {
                return Ok(TaktApiResult<ReplaceWordsResultDto>.Fail("文本不能为空"));
            }
        ...
        {
            return Ok(TaktApiResult<ReplaceWordsResultDto>.Fail($"替换失败: {ex.Message}"));
        }
    }
    ...
    [HttpPost("highlight")]
    [TaktPermission("routine:wordfilter:highlight", "高亮敏感词")]
    [AllowAnonymous] // 允许匿名访问
    public ActionResult<TaktApiResult<HighlightWordsResultDto>> HighlightWords([FromBody] HighlightWordsDto dto)
    {
        try
        {
            if (string.IsNullOrWhiteSpace(dto.Text))
            {
                return Ok(TaktApiResult<HighlightWordsResultDto>.Fail("文本不能为空"));
            }
        ...
        {
            return Ok(TaktApiResult<HighlightWordsResultDto>.Fail($"高亮失败: {ex.Message}"));
        }
    }
    ...
    [HttpGet("stats")]
    [TaktPermission("routine:wordfilter:stats", "查询敏感词统计")]
    public ActionResult<TaktApiResult<WordFilterStatsDto>> GetStats()
    {
        ...
        {
            return Ok(TaktApiResult<WordFilterStatsDto>.Fail($"获取统计信息失败: {ex.Message}"));
        }
    }
    ...
    [HttpGet("groups")]
    [TaktPermission("routine:wordfilter:groups", "查询词库组列表")]
    public async Task<ActionResult<TaktApiResult<List<WordLibraryFileDto>>>> GetWordLibraryFiles()
    {
        try
        {
            ...
            var filePaths = Directory.GetFiles(wordsDirectory, "*.txt", SearchOption.TopDirectoryOnly);
        ...
        {
            return Ok(TaktApiResult<List<WordLibraryFileDto>>.Fail($"获取词库文件列表失败: {ex.Message}"));
        }
    }
    ...
    [HttpGet("words")]
    [TaktPermission("routine:wordfilter:list", "查询敏感词列表")]
    public ActionResult<TaktApiResult<List<string>>> GetAllWords()
    {
        ...
        {
            return Ok(TaktApiResult<List<string>>.Fail($"获取敏感词列表失败: {ex.Message}"));
        }
    }
    ...
    [HttpPost("words")]
    [TaktPermission("routine:wordfilter:add", "添加敏感词")]
    public async Task<ActionResult<TaktApiResult<AddWordsResultDto>>> AddWords([FromBody] AddWordsDto dto)
    {
        try
        {
            ...
            {
                return Ok(TaktApiResult<AddWordsResultDto>.Fail("敏感词列表不能为空"));
            }
            ...
            if (!string.IsNullOrWhiteSpace(dto.Group))
            {
                try
                {
                    ...
                    {
                        return Ok(TaktApiResult<AddWordsResultDto>.Fail("词库文件必须是.txt格式"));
                    }
            ...
            return Ok(TaktApiResult<AddWordsResultDto>.Ok(result, $"成功添加 {result.AddedCount} 个敏感词"));
        }
        catch (Exception ex)
        {
            return Ok(TaktApiResult<AddWordsResultDto>.Fail($"添加敏感词失败: {ex.Message}"));
        }
    }
    ...
    [HttpDelete("words")]
    [TaktPermission("routine:wordfilter:remove", "移除敏感词")]
    public ActionResult<TaktApiResult<RemoveWordsResultDto>> RemoveWords([FromBody] RemoveWordsDto dto)
    {
        try
        {
            if (dto.Words == null || dto.Words.Count == 0)
            {
                return Ok(TaktApiResult<RemoveWordsResultDto>.Fail("敏感词列表不能为空"));
            }
            ...
            return Ok(TaktApiResult<RemoveWordsResultDto>.Ok(result, $"成功移除 {result.RemovedCount} 个敏感词"));
        }
        catch (Exception ex)
        {
            return Ok(TaktApiResult<RemoveWordsResultDto>.Fail($"移除敏感词失败: {ex.Message}"));
        }
    }
    ...
    [HttpDelete("clear")]
    [TaktPermission("routine:wordfilter:clear", "清空敏感词库")]
    public ActionResult<TaktApiResult<object>> ClearWords()
    {
        try
        {
            TaktWordFilterHelper.Clear();
            return Ok(TaktApiResult<object>.Ok(null, "敏感词库已清空"));
        }
        catch (Exception ex)
        {
            return Ok(TaktApiResult<object>.Fail($"清空敏感词库失败: {ex.Message}"));
        }
    }
}
