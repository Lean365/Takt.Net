...
namespace Takt.WebApi.Controllers.Routine.Cache;
...
[TaktPermission("routine:cache", "缓存管理")]
public class TaktCacheController : TaktControllerBase
{
    ...
    /// </summary>
    [HttpGet("info")]
    [TaktPermission("routine:cache:list", "查询缓存信息")]
    ...
    /// </summary>
    [HttpGet("statistics")]
    [TaktPermission("routine:cache:list", "查询缓存统计")]
    public ActionResult<TaktApiResult<TaktCacheStatisticsDto>> GetStatistics()
    {
        ...
        {
            return Ok(TaktApiResult<TaktCacheStatisticsDto>.Ok(new TaktCacheStatisticsDto { Supported = false, Message = "当前提供者为 " + provider + "，仅 Memory 缓存支持统计。" }));
        }
        ...
        if (stats == null)
        {
            return Ok(TaktApiResult<TaktCacheStatisticsDto>.Ok(new TaktCacheStatisticsDto { Supported = true, Message = "统计未启用或暂无数据。" }));
        }
    ...
    /// <param name="key">缓存键</param>
    [HttpDelete]
    [TaktPermission("routine:cache:delete", "移除缓存")]
    public async Task<ActionResult<TaktApiResult>> RemoveAsync([FromQuery] string key)
    {
        if (string.IsNullOrWhiteSpace(key))
            return Ok(TaktApiResult.Fail("缓存键不能为空"));
        await _cache.RemoveAsync(key);
        return Ok(TaktApiResult.Ok("已移除"));
    }
}
