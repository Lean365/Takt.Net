...
using Takt.Application.Services.Routine.Files;
...
namespace Takt.WebApi.Controllers.Routine.Files;
...
[TaktPermission("routine:file", "文件管理")]
public class TaktFilesController : TaktControllerBase
{
    ...
    [HttpGet("list")]
    [TaktPermission("routine:file:list", "查询文件列表")]
    public async Task<ActionResult<TaktPagedResult<TaktFileDto>>> GetListAsync([FromQuery] TaktFileQueryDto queryDto)
    {
        var result = await _fileService.GetListAsync(queryDto);
        return Ok(result);
    }
    ...
    [HttpGet("{id}")]
    [TaktPermission("routine:file:query", "查询文件详情")]
    public async Task<ActionResult<TaktFileDto>> GetByIdAsync(long id)
    {
    ...
    [TaktPermission("routine:file:query", "根据编码查询文件")]
    ...
    /// <returns>文件DTO</returns>
    [HttpPost]
    [TaktPermission("routine:file:create", "创建文件记录")]
    public async Task<ActionResult<TaktFileDto>> CreateAsync([FromBody] TaktFileCreateDto dto)
    {
    ...
    /// <returns>文件DTO</returns>
    [HttpPut("{id}")]
    [TaktPermission("routine:file:update", "更新文件记录")]
    public async Task<ActionResult<TaktFileDto>> UpdateAsync(long id, [FromBody] TaktFileUpdateDto dto)
    {
        try
        {
            var file = await _fileService.UpdateAsync(id, dto);
            return Ok(file);
        }
        catch (Exception ex)
        {
    ...
    [HttpDelete("{id}")]
    [TaktPermission("routine:file:delete", "删除文件记录")]
    ...
    /// <returns>操作结果</returns>
    [HttpDelete("batch")]
    [TaktPermission("routine:file:delete", "批量删除文件记录")]
    ...
    /// <returns>文件DTO</returns>
    [HttpPut("status")]
    [TaktPermission("routine:file:status", "更新文件状态")]
    public async Task<ActionResult<TaktFileDto>> UpdateStatusAsync([FromBody] TaktFileStatusDto dto)
    {
        try
        {
            var file = await _fileService.UpdateStatusAsync(dto);
            return Ok(file);
        }
        catch (Exception ex)
        {
    ...
    /// <returns>文件DTO</returns>
    [HttpPut("change")]
    [TaktPermission("routine:file:change", "切换文件公开/私有状态")]
    ...
    [HttpPost("upload")]
    [TaktPermission("routine:file:upload", "上传文件")]
    ...
    [HttpPost("export")]
    [TaktPermission("routine:file:export", "导出文件列表")]
    public async Task<IActionResult> ExportAsync([FromBody] TaktFileQueryDto query, [FromQuery] string? sheetName = null, [FromQuery] string? fileName = null)
    {
        try
        {
            var (resultFileName, content) = await _fileService.ExportAsync(query, sheetName, fileName);
    ...
    [TaktPermission("routine:file:download", "增加下载次数")]
    ...
    [HttpGet("{id}/download")]
    [TaktPermission("routine:file:download", "下载文件")]
