        ...
        try
        {
            ...
            foreach (var el in doc.RootElement.EnumerateArray())
            {
                ...
                else if (el.ValueKind == JsonValueKind.Object)
                {
                    ...
                    if (string.IsNullOrEmpty(name)) continue;
                    var bodyKind = el.TryGetProperty("BodyKind", out var bk) ? bk.GetString() ?? "" : "";
                    var baseClass = el.TryGetProperty("BaseClass", out var bc) ? bc.GetString() : null;
