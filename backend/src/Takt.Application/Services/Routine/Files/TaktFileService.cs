...
using Takt.Application.Dtos.Routine.Files;
using Takt.Domain.Entities.Routine.Files;
using Takt.Domain.Interfaces;
using Takt.Domain.Validation;
using Takt.Shared.Exceptions;
using Takt.Shared.Helpers;

namespace Takt.Application.Services.Routine.Files;
...
public class TaktFileService : TaktServiceBase, ITaktFileService
{
    ...
    public async Task<TaktPagedResult<TaktFileDto>> GetListAsync(TaktFileQueryDto queryDto)
    {
    ...
    /// <returns>文件DTO</returns>
    public async Task<TaktFileDto> CreateAsync(TaktFileCreateDto dto)
    {
        // 查重验证（文件编码唯一）
        await TaktUniqueValidatorExtensions.ValidateUniqueAsync(_fileRepository, f => f.FileCode, dto.FileCode, null, $"文件编码 {dto.FileCode} 已存在");
        ...
        return await GetByIdAsync(file.Id) ?? file.Adapt<TaktFileDto>();
    }
    ...
    /// <returns>文件DTO</returns>
    public async Task<TaktFileDto> UpdateAsync(long id, TaktFileUpdateDto dto)
    {
        var file = await _fileRepository.GetByIdAsync(id);
        if (file == null)
        ...
        // 查重验证（文件编码唯一，排除当前记录）
        await TaktUniqueValidatorExtensions.ValidateUniqueAsync(_fileRepository, f => f.FileCode, dto.FileCode, id, $"文件编码 {dto.FileCode} 已存在");
        ...
        return await GetByIdAsync(id) ?? file.Adapt<TaktFileDto>();
    }
    ...
    /// <returns>文件DTO</returns>
    public async Task<TaktFileDto> UpdateStatusAsync(TaktFileStatusDto dto)
    {
        ...
        return await GetByIdAsync(dto.FileId) ?? file.Adapt<TaktFileDto>();
    }
    ...
    public async Task<(string fileName, byte[] content)> DownloadAsync(long id)
    {
        ...
        if (string.IsNullOrWhiteSpace(file.FilePath))
            throw new TaktBusinessException("文件路径不存在");
        ...
        try
        {
            ...
            switch (file.StorageType)
            {
                case 0: // 本地存储
                    ...
                    break;
                case 1: // OSS对象存储
                    throw new TaktBusinessException("OSS对象存储下载功能未实现");
                case 2: // FTP
                    throw new TaktBusinessException("FTP存储下载功能未实现");
                default:
                    throw new TaktBusinessException($"未知的存储类型: {file.StorageType}");
            }

            if (fileStream == null)
                throw new TaktBusinessException("文件流为空");
    ...
    private Task<Stream> DownloadLocalFileAsync(string filePath)
    {
        if (string.IsNullOrWhiteSpace(filePath))
            throw new TaktBusinessException("文件路径为空");
        ...
        else
        {
            // 相对路径，需要构建完整路径
            if (_webHostEnvironment == null)
                throw new TaktBusinessException("IWebHostEnvironment 未注入，无法读取相对路径文件");
        ...
        if (!File.Exists(fullPath))
            throw new TaktBusinessException($"文件不存在: {filePath}");
    ...
    /// <returns>文件名和文件内容</returns>
    public async Task<(string fileName, byte[] content)> ExportAsync(TaktFileQueryDto query, string? sheetName, string? fileName)
    {
        // 构建查询条件
        ...
        if (files == null || files.Count == 0)
        {
            ...
            return await TaktExcelHelper.ExportAsync(
                ...
                string.IsNullOrWhiteSpace(sheetName) ? "文件数据" : sheetName,
                string.IsNullOrWhiteSpace(fileName) ? "文件导出" : fileName
            );
        }
    ...
    private static Expression<Func<TaktFile, bool>> QueryExpression(TaktFileQueryDto queryDto)
    {
        ...
        // 创建时间范围
        exp = exp.AndIF(queryDto?.CreateTimeStart.HasValue == true, x => x.CreateTime >= queryDto!.CreateTimeStart!.Value);
        exp = exp.AndIF(queryDto?.CreateTimeEnd.HasValue == true, x => x.CreateTime <= queryDto!.CreateTimeEnd!.Value);
