...
public class TaktUserProvider : ITaktUserContext
{
    ...
    public string? GetCurrentUserName()
    {
        ...
        // 如果从 Claims 无法获取用户名，但用户已认证，尝试从数据库加载用户实体
        // 注意：这是同步方法，但为了确保 SignalR Hub 能获取用户名，需要同步加载
        if (string.IsNullOrEmpty(userName))
        {
            var userId = GetCurrentUserId();
            if (userId.HasValue)
            {
                // 使用 Task.Run 同步等待异步操作（避免死锁）
                // 直接使用数据库客户端，避免循环依赖
                try
                {
                    var db = _dbContext.GetClient(typeof(TaktUser));
                    var user = Task.Run(async () => await db.Queryable<TaktUser>()
                        .Where(u => u.Id == userId.Value && u.IsDeleted == 0)
    ...
    public string? GetCurrentRealName()
    {
        ...
        if (TaktUserContext.CurrentUser != null)
        {
            return TaktUserContext.CurrentUser.RealName;
        }
        ...
        return httpContext.User.FindFirst(ClaimTypes.GivenName)?.Value;
    }
    ...
    {
        return TaktUserContext.CurrentUser?.NickName;
    }
    ...
    {
        return TaktUserContext.CurrentUser?.Avatar;
    }
    ...
    {
        return TaktUserContext.CurrentUser?.Gender;
    }
