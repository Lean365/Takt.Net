...
public class TaktExcelHelper
{
    ...
    private static void SetWorkbookProperties(ExcelWorkbook workbook)
    {
        ...
        if (_options == null)
        {
            ...
            throw new InvalidOperationException("TaktExcelHelper 配置未初始化，请在应用程序启动时调用 TaktExcelHelper.Configure() 方法");
        }
        ...
        string sheetName = "Data") where T : class, new()
    {
        ...
        try
        {
            ...
            var result = ImportFromSheetAsync<T>(package, sheetName).Result;
    ...
    /// <param name="sheetName">工作表名称</param>
    /// <param name="fileName">文件名（仅名称，不含 .xlsx；为空时用类型名+模板；后端自动拼接 名称_yyyyMMddHHmmss.xlsx）</param>
    /// <returns>最终文件名与内容（fileName 已含时间戳与 .xlsx）</returns>
        ...
        string? fileName = null) where T : class, new()
    {
        ...
        var finalFileName = GenerateTimestampFileName(actualFileName);
    ...
    private static Task<List<T>> ImportFromSheetAsync<T>(ExcelPackage package, string sheetName) where T : class, new()
    {
        ...
        if (worksheet == null || worksheet.Dimension == null)
            throw new InvalidOperationException($"找不到名为 {sheetName} 的工作表或工作表为空");
