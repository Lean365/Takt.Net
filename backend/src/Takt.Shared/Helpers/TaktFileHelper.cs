...
public static class TaktFileHelper
{
    ...
    /// <param name="throwIfNotExists">如果文件不存在是否抛出异常，默认为false</param>
    /// <returns>是否删除成功</returns>
    public static Task<bool> DeleteFileAsync(string filePath, bool throwIfNotExists = false)
    {
        return Task.FromResult(DeleteFile(filePath, throwIfNotExists));
    }
    ...
    /// <param name="overwrite">是否覆盖已存在的文件，默认为false</param>
    /// <returns>任务</returns>
    public static Task CopyFileAsync(string sourcePath, string destinationPath, bool overwrite = false)
    {
        try
        {
    ...
    /// <param name="overwrite">是否覆盖已存在的文件，默认为false</param>
    /// <returns>任务</returns>
    public static Task MoveFileAsync(string sourcePath, string destinationPath, bool overwrite = false)
    {
        try
        {
