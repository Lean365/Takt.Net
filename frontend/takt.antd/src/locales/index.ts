...
const i18n = createI18n({
  legacy: false,
  locale: localStorage.getItem('locale') || 'zh-CN', // 默认使用中文
  fallbackLocale: 'zh-CN', // 后备语言使用中文
  messages: staticMessages,
  // 配置缺失翻译键的处理：只在生产环境关闭警告，开发环境保留但减少重复警告
  missingWarn: import.meta.env.DEV,
  // 自定义缺失翻译键的处理函数，避免重复警告
  missing: (_locale: string, key: string, _vm?: any, _values?: any) => {
    // 如果是在开发环境，且键包含 menu.，说明是菜单翻译，可能是后端数据还未加载
    // 这种情况下不输出警告，因为后端数据会在路由加载后异步加载
    if (import.meta.env.DEV && key.startsWith('menu.')) {
      // 静默处理，不输出警告
      return key
    }
    // 其他情况返回 key 作为默认值
