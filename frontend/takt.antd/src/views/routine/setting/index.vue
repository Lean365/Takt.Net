...
const columns = ref<TableColumnsType>([
  ...
  CreateActionColumn({
    actions: [
      { key: 'update', label: '编辑', shape: 'plain', icon: RiEditLine, permission: 'routine:setting:update', onClick: (r: Setting) => handleEdit(r) },
      { key: 'delete', label: '删除', shape: 'plain', icon: RiDeleteBinLine, permission: 'routine:setting:delete', onClick: (r: Setting) => handleDeleteOne(r) }
    ]
  })
])
...
function handleDownloadTemplate() {
  getSettingTemplate(undefined, '设置导入模板.xlsx').then(blob => {
    const url = window.URL.createObjectURL(blob)
    const link = document.createElement('a')
    link.href = url
    link.download = '设置导入模板.xlsx'
    link.click()
